    ...
    "cells": [
        ...
        },
        {
            ...
            "source": [
                ...
                "        return []\n",
                "\n",
                "async def get_pdfs_tocs(client, folder_path, offsets_dict=None):\n",
...
"            videos = []\n",
                ...
                "                return json.load(f)\n",
                "        except Exception as e:\n",
                "            logging.warning(f\"Erro ao ler offsets.json em {refs_dir}: {e}\")\n",
                "    return {}\n",
                "\n",
                "def salvar_offsets(offsets, refs_dir):\n",
                "    offsets_path = os.path.join(refs_dir, 'offsets.json')\n",
                "    try:\n",
                "        with open(offsets_path, 'w', encoding='utf-8') as f:\n",
                "            json.dump(offsets, f, ensure_ascii=False, indent=2)\n",
                "    except Exception as e:\n",
                "        logging.warning(f\"Erro ao salvar offsets.json em {refs_dir}: {e}\")\n",
                "\n",
                "def pedir_offset(arquivo, caminho):\n",
                "    try:\n",
                ...
                "    return offset, total\n",
                "\n",
                "def mapear_offsets(pasta_livros):\n",
                "    offsets = carregar_offsets(pasta_livros)\n",
                "    livros  = sorted([f for f in os.listdir(pasta_livros) if f.lower().endswith('.pdf')])\n",
                "    if not livros:\n",
                "        print('⚠️  Nenhum PDF encontrado em:', pasta_livros)\n",
                "        return offsets\n",
                "\n",
                "    print('=' * 65)\n",
                "    print('📐 OFFSETS DE PÁGINA (offsets.json)')\n",
                "    print('=' * 65)\n",
                "    algum_novo = False\n",
                ...
                "                offset_novo, total_novo = pedir_offset(arquivo, caminho)\n",
                ...
                "        else:\n",
                "            algum_novo = True\n",
                "            offset, total = pedir_offset(arquivo, caminho)\n",
                "            offsets[arquivo] = {'offset': offset, 'total_paginas': total}\n",
                "    salvar_offsets(offsets, pasta_livros)\n",
                "    print()\n",
                "    print('=' * 65)\n",
                "    print('📋 OFFSETS MAPEADOS:')\n",
                "    print('=' * 65)\n",
                "    for arquivo in livros:\n",
                "        entrada = offsets[arquivo]\n",
                "        if isinstance(entrada, dict):\n",
                ...
                "    print(f'💾 offsets.json salvo na pasta dos livros!')\n",
                "    return offsets\n",
                "\n",
                "def obter_offset_pdf(source_pdf, offsets_dict=None):\n",
                "    if offsets_dict is None:\n",
                "        refs_dir = os.path.dirname(source_pdf)\n",
                "        offsets_dict = carregar_offsets(refs_dir)\n",
                "    filename = os.path.basename(source_pdf)\n",
                "    if filename in offsets_dict:\n",
                "        entrada = offsets_dict[filename]\n",
                "        if isinstance(entrada, dict):\n",
                ...
                "\n",
                "def gerar_pdfs(roteiro, pdfs_dir, output_dir, tocs=None, reconciliar=True):\n",
                ...
                "    return None\n",
                "\n",
                "async def process_roteiro(objetivos_text, tocs, refs_dir, offsets_dict=None):\n",
                "    if offsets_dict is None:\n",
                "        offsets_dict = carregar_offsets(refs_dir)\n",
                "    contexto = \"SUMÁRIOS:\\n\"\n",
                ...
                "    try:\n",
                "        offset = obter_offset_pdf(pdf_path, OFFSETS_MANUAIS)\n",
                "        doc = fitz.open(pdf_path)\n",
                "        text = \"\"\n",
                "        start = max(0, p_ini + offset - 1)\n",
                "        end = min(len(doc), max(p_ini + offset, p_fim + offset))\n",
                ...
                "    m = re.match(r'PT(?:(\\d+)H)?(?:(\\d+)M)?(?:(\\d+)S)?', duration_str, re.IGNORECASE)\n",
                "    if not m: return 0\n",
                ...
                "    search_url = f\"https://www.googleapis.com/youtube/v3/search?part=snippet&q={query}&type=video&maxResults=10&key={api_key}&relevanceLanguage=pt\"\n",
                ...
                "                if r_det.status_code != 200: return []\n",
                ...
                "                        continue\n",
                "                    dur_sec = parse_iso_duration(content_details.get('duration', ''))\n",
                "                    if dur_sec < 600:\n",
                "                        logging.info(f\"Desconsiderando vídeo curto ({dur_sec // 60} min): '{title}'\")\n",
                "                        continue\n",
                ...
                "\n",
                "**CONTEXTO:**\n",
                "Você receberá o TEMA / TERMO DE BUSCA e opções pré-filtradas de vídeos longos (>= 10 minutos) retornadas pelo YouTube.\n",
                "\n",
                "**AÇÕES:**\n",
                "1. Analise o Tema/Termo para entender o foco clínico exigido.\n",
                "2. Varra os metadados (Título, Canal, Duração, Descrição) de cada vídeo disponível.\n",
                ...
                "4. Eleja a opção de maior profundidade científica e extensão adequada.\n",
                ...
                "2. **Filtro de Duração Mínima:** REJEITE vídeos com menos de 10 minutos de duração ou cortes/shorts incompletos.\n",
                "3. **Filtro de Idioma (ESTRITO):** É TERMINANTEMENTE PROIBIDO selecionar vídeos em inglês, espanhol ou qualquer idioma estrangeiro. A videoaula DEVE ser estritamente em Português do Brasil (PT-BR). Se todas as opções forem em idioma estrangeiro, defina o `video_escolhido_id` obrigatoriamente como 'NENHUM'.\n",
                "4. **Alucinação Zero:** NUNCA invente um ID de vídeo que não esteja explicitamente listado nas opções. O ID retornado DEVE ser uma das opções fornecidas.\n",
                "5. **Formato de Saída:** Retorne estritamente o objeto JSON conforme o schema exigido (CuradoriaVideo).\n",
                "\n",
                "**SAÍDA:**\n",
        ...
        },
        {
            ...
            "outputs": [],
            "source": [
                "# 5. MAPEAR OFFSETS DOS LIVROS (INTERATIVO)\n",
                "# ========================================\n",
                "# Rode esta célula ANTES de gerar o roteiro para garantir\n",
                "# que as páginas extraídas batam exatamente com o livro físico.\n",
                "PASTA_LIVROS = \"/content/drive/MyDrive/Logística - Drive/Tutoria\"\n",
                "mapear_offsets(PASTA_LIVROS)\n"
            ]
        },
        {
            "cell_type": "code",
            ...
            "outputs": [],
            "source": [
                "# 6. INSERIR OBJETIVOS E GERAR ESBOÇO\n",
                "# ===================================\n",
                "PASTA_LIVROS = \"/content/drive/MyDrive/Logística - Drive/Tutoria\"\n",
        ...
        },
        {
            ...
            "outputs": [],
            "source": [
                "# 7. EXPORTAR PDFs FINAIS (Após sua revisão!)\n",
                "# ==========================================\n",
                "import json\n",
                "import os\n",
                "import logging\n",
                ...
                "    gerar_pdfs(roteiro, PASTA_LIVROS, PASTA_SAIDA, reconciliar=False)\n",
