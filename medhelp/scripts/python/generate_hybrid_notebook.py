"""
generate_hybrid_notebook.py
===========================
Gera o Orquestrador Híbrido (Orquestrador_Hibrido.ipynb) que funde:
  - Input manual via NotebookLM (texto bruto → JSON via Gemini API)
  - Curadoria de vídeos YouTube (Agente YouTube do Orquestrador Automático)
  - Capa premium com Índice + Vídeos (ReportLab platypus do Roteiro Tutoria)
  - Fatiamento direto de PDF sem releitura (sem Agente 2)
"""
import json
import os
...
def code(source_lines):
    """Cria célula de código."""
    return {"cell_type": "code", "execution_count": None, "metadata": {}, "outputs": [], "source": source_lines}

def lines(*args):
    """Junta strings adicionando \\n no final de cada uma, exceto a última."""
    result = []
...
# ── CÉLULA 1: Setup ────────────────────────────────────────────────────────────
cells.append(md(lines("## 🔧 Célula 1 — Instalação e Montagem")))
...
cells.append(code(lines(
    ...
    "    return None",
    "",
    "# Versão com schema Pydantic para curadoria de vídeos",
    "import pydantic",
    "",
    ...
    "    s_pg_right = ParagraphStyle('pg_right', fontName='Helvetica-Bold', fontSize=9.5,",
    "                                textColor=colors.HexColor('#2b8a3e'), alignment=2, leading=14)",
    "    s_livro  = ParagraphStyle('livro',  fontName='Helvetica-Bold', fontSize=10,",
    "                              textColor=preto, spaceAfter=1, leading=14, wordWrap='LTR')",
    "    s_cap    = ParagraphStyle('cap',    fontName='Helvetica', fontSize=9.5,",
    "                              textColor=cinza, spaceAfter=1, leading=13,",
    "                              leftIndent=0, wordWrap='LTR')",
    ...
    "                                    textColor=azul, spaceAfter=4, leading=11, letterSpacing=0.5)",
    "    s_fusao_texto  = ParagraphStyle('fusao_texto', fontName='Helvetica', fontSize=9,",
    "                                    textColor=preto, spaceAfter=2, leading=13,",
    "                                    leftIndent=8, wordWrap='LTR')",
    "    s_vid_header = ParagraphStyle('vid_header', fontName='Helvetica-Bold', fontSize=9,",
    "                                  textColor=azul, spaceBefore=12, spaceAfter=6,",
    "                                  leading=12, letterSpacing=0.5)",
    "    s_vid_link   = ParagraphStyle('vid_link', fontName='Helvetica', fontSize=10,",
    ...
    "        for linha in linhas:",
    "            elems.append(Paragraph(f'• {html.escape(linha)}', s_fusao_texto))",
    ...
    "",
    "        col_esq = [Paragraph(html.escape(nome), s_livro)]",
    "        if capitulo:",
    "            col_esq.append(Paragraph(html.escape(capitulo), s_cap))",
    "",
    "        col_dir = Paragraph(pg_str, s_pg_right)",
    "",
    ...
    "",
    "    y_apos = draw_wrapped(c, nome_livro, x=x_texto, y_topo=base + 1.8 * cm,",
    "        fonte='Helvetica-Bold', tamanho=18, cor=preto, line_height=0.75 * cm)",
    "    if capitulo:",
    "        y_apos = draw_wrapped(c, capitulo, x=x_texto, y_topo=y_apos - 0.4 * cm,",
    "            fonte='Helvetica-Bold', tamanho=11, cor=preto, line_height=0.45 * cm)",
    "    if secao:",
    "        draw_wrapped(c, f'›  {secao}', x=x_texto, y_topo=y_apos - 0.3 * cm,",
    "            fonte='Helvetica', tamanho=10, cor=cinza, line_height=0.4 * cm)",
    "",
    "    c.save()",
    "    return buffer",
    "",
...
# ── CÉLULA 6: Converter NotebookLM → JSON ─────────────────────────────────────
cells.append(md(lines(
...
notebook = {
    ...
    "metadata": {
        "colab": {
            "name": "Orquestrador_Hibrido.ipynb",
            "provenance": []
        },
        "kernelspec": {
...
output_dir = os.path.join(os.path.dirname(__file__), '..', 'colab')
os.makedirs(output_dir, exist_ok=True)
output_path = os.path.join(output_dir, 'Orquestrador_Hibrido.ipynb')

with open(output_path, 'w', encoding='utf-8') as f:
    json.dump(notebook, f, ensure_ascii=False, indent=2)

print(f"Notebook gerado e salvo com sucesso em {os.path.abspath(output_path)}")
